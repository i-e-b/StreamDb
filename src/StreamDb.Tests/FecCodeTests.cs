...
namespace StreamDb.Tests
{
    ...
    {
        [TestFixture]
        public class OverlapParityTests
        {
            /// <summary>
            ...
            public void overlapping_bitwise_parity()
            {
                ...
                var confidence = ConfidenceSketch(message, parityMatrix) ?? throw new Exception();
            ...
            private static float[] ConfidenceSketch(byte[] message, int[] parityMatrix)
            {
                // NOTE: This is all wrong, and we can't really gain any repair from it.

                if (message == null || parityMatrix == null) return null;
